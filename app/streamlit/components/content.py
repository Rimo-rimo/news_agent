...
def render_content(user_info, text_font_size):
    ...
    try:
            ...
            with st.container(border=False):
                with st.spinner("##### 뉴스를 읽어보고 있어요"):
                    ...
                    if len(news_content) < 30:
                        st.error("뉴스를 가져오는데 실패했어요..😅 다른 뉴스로 다시 시도해 주세요.")
                        st.stop()
        ...
        read_image_empty.empty()  # 로딩 애니메이션 제거
        if "CrawlAgent" in str(e.__class__):
            st.error("뉴스를 가져오는데 실패했어요..😅 잠시 후 다시 시도해주세요.")
        elif "PreNewsAgent" in str(e.__class__):
            st.error("뉴스 내용을 분석하는데 실패했어요..😅 잠시 후 다시 시도해주세요.")
        else:
            st.error("예상치 못한 오류가 발생했어요..😅 잠시 후 다시 시도해주세요.")
        st.stop()  # return 대신 st.stop() 사용

    # Display title and introduction
    if newsletter_title and introduction:
        with content_container:
            st.markdown(
                f"""<h3 style='text-align: center; width: 100%;'>
                    <a href="{st.session_state.news_query}" target="_blank" style="color: #333D4B; text-decoration: none; cursor: pointer;">
                        {newsletter_title}
                    </a>
                </h3>""",
        ...
        with qa_container:
            with st.container(border=False):
                ...
                try:
                    ...
                    search_image_empty.empty()  # 로딩 애니메이션 제거
                    st.error("추가 정보를 검색하는데 실패했어요..😅 잠시 후 다시 시도해주세요.")
                    print(e)
                    st.stop()  # return 대신 st.stop() 사용
    ...
    with newsletter_container:
        with st.container(border=False):
            ...
            try:
                ...
                write_image_empty.empty()  # 로딩 애니메이션 제거
                st.error("뉴스레터 작성에 실패했어요..😅 잠시 후 다시 시도해주세요.")
                st.stop()  # return 대신 st.stop() 사용
        st.text(" ")
        st.text(" ")
        st.text(" ")